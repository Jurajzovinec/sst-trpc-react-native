...
import type { APIGatewayProxyWithCognitoAuthorizerEvent, APIGatewayProxyWithCognitoAuthorizerHandler } from 'aws-lambda'
...
export const handler: APIGatewayProxyWithCognitoAuthorizerHandler = (event, context) =>
	awsLambdaRequestHandler({
		router: appRouter,
		responseMeta: () => ({
			headers: {
				'Access-Control-Allow-Origin': `${
					(process.env['ALLOW_ORIGINS'] ?? '').split(';').includes(event.headers['origin'] ?? 'none')
